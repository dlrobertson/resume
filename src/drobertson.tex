\documentclass[a4paper,skipsamekey,11pt,english]{curve}

\usepackage[empty]{fullpage}

\usepackage{titlesec}
\usepackage{fancyhdr}
\usepackage{fontawesome5}

\usepackage{multicol}
\usepackage{bookmark}

% fonts
\usepackage[T1]{fontenc}
\usepackage[p,osf,swashQ]{cochineal}
\usepackage{cabin}
\usepackage[varqu,varl,scale=0.9]{zi4}

% maybe use this?
\usepackage[most]{tcolorbox}

% colors
\usepackage[table]{xcolor}
\definecolor{base1}{HTML}{93a1a1}
\definecolor{base03}{HTML}{002b36}
\definecolor{green}{HTML}{859900}
\definecolor{blue}{HTML}{268bd2}
\definecolor{orange}{HTML}{cb4b16}
\definecolor{red}{HTML}{dc322f}

\pagestyle{fancy}
\fancyhf{}
\fancyfoot{}
\renewcommand{\headrulewidth}{0pt}
\renewcommand{\footrulewidth}{0pt}
\urlstyle{same}

\addtolength{\oddsidemargin}{-0.7in}
\addtolength{\evensidemargin}{-0.5in}
\addtolength{\textwidth}{1.19in}
\addtolength{\topmargin}{-0.7in}
\addtolength{\textheight}{1.4in}

\setlength{\multicolsep}{-3.0pt}
\setlength{\columnsep}{-1pt}
\setlength{\tabcolsep}{0pt}
\setlength{\footskip}{3.7pt}
\raggedbottom
\raggedright

\input{glyphtounicode}
\pdfgentounicode=1

\hypersetup{
  colorlinks=false,
  pdfborder={0 0 0},
}

\newcommand{\allTheInfo}[3]{
  \begin{center}
    {\Huge\color{red} #1} \hfill {
      \begin{minipage}{3in}
        #2
      \end{minipage}
    }
    \vspace{10pt}
    \begin{tcolorbox}[
      sharp corners,
      boxrule=0mm,
      enhanced,
      borderline north={1pt}{0pt}{blue},
      borderline south={1pt}{0pt}{blue},
      borderline west={0pt}{0pt}{base1!10},
      borderline east={0pt}{0pt}{base1!10},
    ]
      \begin{center}
        \vspace{-5pt}
        \Large{#3}
        \vspace{-5pt}
      \end{center}
    \end{tcolorbox}
  \end{center}
}

\newcommand{\makeSkillRow}[3]{
  {$\Rightarrow$#1} & {$\Rightarrow$#2} & {$\Rightarrow$#3}\\
}

\newcommand{\makeWorkHistoryTitle}[5]{
  \begin{tcolorbox}[colback=base1!10,colframe=base1!10,frame hidden]
    {\color{blue}\textbf{#1}}, \textbf{#2} - #3 \hfill #4
  \end{tcolorbox}
}

\newcommand{\makeFooter}[4]{
  \fancyfoot[R]{
    \color{base1}{
      #1 ~ | ~ #2 ~ | ~ #3 ~ | ~ #4 ~ | ~ $\thepage$ of $2$
    }
  }
}


\titleformat
{\section} % command
[display] % shape
{\bfseries\Large\itshape} % format
{} % label
{0em} % sep
{
  \vspace{-5pt}
  \color{green}
  \raggedright\Large\bfseries
} % before-code
[
  \vspace{-10pt}
  {
    % Stollen from the example resume by LianTze Lim (liantze@gmail.com)...
    % WAY cooler than a simple \hline.
    \begin{tikzpicture}[baseline]
      \shade[left color=blue!60!white, right color=white] rectangle (17,2.5pt);
    \end{tikzpicture}
  }
] % after-code

% I'm not a wizard, so I can't call this a
% Curriculum Vitae
\title{Resume}

\begin{document}

  \allTheInfo{
    Daniel Robertson
  }{
    \href{tel:5714519241}{
      \raisebox{-0.05\height} \faPhone\ 571-451-9241} ~ | ~
    \href{mailto:dan@dlrobertson.com}{
      \raisebox{-0.15\height} \faEnvelope\ dan@dlrobertson.com}\\
    \href{www.linkedin.com/in/daniellutherrobertson/ }{
      \raisebox{-0.15\height} \faLinkedin\ @daniellutherrobertson }\\
    \href{https://github.com/dlrobertson}{
      \raisebox{-0.15\height} \faGithub\ @dlrobertson} ~ | ~
    \href{https://dlrobertson.com}{
      \raisebox{-0.15\height} \faGlobe\ https://dlrobertson.com}
  }{
    \color{base03}\textbf{Software Developer}
  }

  \begin{center}
    \setlength{\tabcolsep}{1em}
    \begin{tabular}{ l l l }
      \makeSkillRow{
        Linux Kernel Development
      }{
        C
      }{
        Linux Kernel Debugging (ftrace)
      }
      \makeSkillRow{
        LSM Development
      }{
        Rust
      }{
        Network Protocols (IPv6)
      }
      \makeSkillRow{
        File System Development
      }{
        X86 Assembly
      }{
        eBPF
      }
      \makeSkillRow{
        Userspace Debugging (rr \& gdb)
      }{
        Fuzz Testing
      }{
        Hypervisor Development
      }\\
    \end{tabular}
  \end{center}

  \section{Upstream Open Source Contributions}
  \begin{center}
    {\rowcolors{2}{white}{base1!10}
    \setlength{\tabcolsep}{1em}
    \begin{tabular}{c!{\color{blue}\vrule}c}
      \arrayrulecolor{blue}
      \vspace{1pt}
      {
       \begin{tabular}{c}
         \color{base03}\textbf{Linux Kernel}
       \end{tabular}
      } & {
        \begin{minipage}{5in}
          \vspace{5pt}
          \begin{itemize}
            \item \textbf{Authored and upstreamed the BMA400 Accelerometer IIO
            driver} to the Linux kernel, enabling use of the device with either
            I2C or SPI serial communication protocols.
            \item \textbf{Improved system stability and robustness} of
            \texttt{bcachefs} by identifying and resolving several kernel
            bugs.
          \end{itemize}
        \end{minipage}
      }
      \\
      \hline
      {
       \begin{tabular}{c}
         \color{base03}\textbf{Rust Compiler}
       \end{tabular}
      } & {
        \begin{minipage}{5in}
          \vspace{5pt}
          \begin{itemize}
            \item \textbf{Closed critical gap in interoperability with C} by
            implementing C \texttt{va\_list} and variadic functions, enhancing
            ability to export C APIs.
            \item \textbf{Optimized stability and functionality of Rust language}
            by rectifying several internal compiler errors.
          \end{itemize}
        \end{minipage}
        \vspace{1pt}
      }
      \\
      \hline
      {
       \begin{tabular}{c}
         \color{base03}\textbf{Smoltcp}
       \end{tabular}
      } & {
        \begin{minipage}{5in}
          \vspace{5pt}
          \begin{itemize}
            \item \textbf{Implemented IPv6 support}, refining Redox OS’
            networking capabilities.
            \item \textbf{Supported project maintenance} and coordinated
            contributors, fostering collaboration in open-source environment.
          \end{itemize}
        \end{minipage}
        \vspace{1pt}
      }
      \\
      \hline
      {
       \begin{tabular}{c}
         \color{base03}\textbf{Mythril}
       \end{tabular}
      } & {
        \begin{minipage}{5in}
          \vspace{5pt}
          \begin{itemize}
            \item \textbf{Designed guest ACPI table building}, enabling virtual
            machines to accurately detect and allocate system resources.
            \item \textbf{Directed initial development efforts} and delegated
            smaller tasks, cultivating teamwork among volunteers.
          \end{itemize}
        \end{minipage}
        \vspace{1pt}
      }
      \\
    \end{tabular}}
  \end{center}

  \section{Work Experience}
  \makeWorkHistoryTitle
    {Senior Software Engineer}
    {MOZILLA}
    {San Francisco, CA (Remote)}
    {Apr. 2022–Present}

  \begin{itemize}
    \item \textbf{Implemented new web functionality} by implementing the new
    \texttt{scrollend} event implementation in Firefox.
    \item \textbf{Provided other teams with Linux Kernel expertise} for bugs
    that required significant knowledge of the internals of the Linux Kernel.
    \item \textbf{Improved web compatibility across multiple browsers} by
    contributing to web speifications and test frameworks, with a focus on
    \texttt{Blob} handling in the \texttt{fetch} specification.
  \end{itemize}

  \makeWorkHistoryTitle
    {Member of the Technical Staff}
    {STAR LAB, A WIND RIVER COMPANY}
    {Washington, DC}
    {Apr. 2019–Apr. 2022}

  \begin{itemize}
    \item \textbf{Improved correctness of the file system implementation}
    by debugging and implementing fixes for issues found when guiding
    customers through the integration process.
    \item \textbf{Developed a custom eBPF LSM} written in Rust to enhance
    customers security posture.
    \item \textbf{Boosted Linux security module performance} by developing the
    customkernel LSM implementation, to evolve from experimental phase to tangible,
    sellable product.
  \end{itemize}

  \newpage

  \makeWorkHistoryTitle
    {Machine Intelligence Engineer}
    {EMBEDDED INTELLIGENCE}
    {Washington, DC}
    {May 2018–Apr. 2019}

  \begin{itemize}
    \item \textbf{Strengthened research proposals} by leveraging in-depth
    knowledge of network protocols.
    \item \textbf{Implemented network protocol parsers} in C to enable
    fast processing of network packets to be analyzed.
  \end{itemize}

  \makeWorkHistoryTitle
    {Software Engineer}
    {TRIPWIRE}
    {Atlanta, GA}
    {May 2016–May. 2018}

  \begin{itemize}
    \item \textbf{Improved system reliability and minimized downtime for customers}
    by resolving complex memory issues in network devices using remote debugging
    techniques and code analysis to identify and fix problems.
    \item \textbf{Implemented new functionality to profile SMB servers} by
    creating new libraries making use of SMB over TCP.
  \end{itemize}

  \makeWorkHistoryTitle
    {ORISE Fellow}
    {CENTERS FOR DISEASE CONTROL \& PREVENTION (CDC)}
    {Atlanta, GA}
    {Sept. 2014–May 2016}

  \begin{itemize}
    \item \textbf{Cut data entry time by 50\%} by writing software
    utilizing computer vision to automatically input survey data
    into forms.
  \end{itemize}

  \section{Education}
  \begin{tcolorbox}[colback=base1!10,colframe=base1!10,frame hidden]
    {\color{base03}\textbf{GEORGE MASON UNIVERSITY}} ~ – ~ Fairfax, VA\\
    \small Master of Public Health in Epidemiology ~ | ~
    Graduate Certificate in Biostatistics ~ | ~
    Bachelor of Science in Community Health
  \end{tcolorbox}

  \section{Conference Presentations}
  \begin{tcolorbox}[colback=base1!10,colframe=base1!10,frame hidden]
    Daniel L. Robertson, Jin-Mann S. Lin. Application of computer vision and
    machine learning to public health data validation. CDC/ATSDR Statistics
    Day. Atlanta, GA. September 22, 2015
  \end{tcolorbox}

  \makeFooter{Daniel Robertson}{Waynesboro, VA}{571-451-9241}{dan@dlrobertson.com}
\end{document}
